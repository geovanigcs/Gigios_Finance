...
export default function TransactionsPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/transactions?id=${deletingId}`, {
        method: "DELETE"
      })
  ...
  // Separar transações do mês atual e mês anterior
  ...
  const currentMonthGroup = monthlyGroups.find(g => format(g.monthDate, "yyyy-MM") === currentMonthKey)
  const lastMonthGroup = monthlyGroups.find(g => format(g.monthDate, "yyyy-MM") === lastMonthKey)
  const otherMonthsGroups = monthlyGroups.filter(g => {
    const key = format(g.monthDate, "yyyy-MM")
    return key !== currentMonthKey && key !== lastMonthKey
  })
